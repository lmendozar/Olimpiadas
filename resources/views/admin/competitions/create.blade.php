    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white shadow-lg rounded-lg overflow-hidden">
            <div class="px-6 py-4 bg-yellow-600">
                <h1 class="text-2xl font-bold text-white">Crear Competencia</h1>
            </div>

            <form action="{{ route('admin.competitions.store') }}" method="POST" class="px-6 py-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <select name="game_type_id" id="game_type_id" required
                        ...
                        @foreach($gameTypes as $gameType)
                            <option value="{{ $gameType->id }}" {{ old('game_type_id') == $gameType->id ? 'selected' : '' }}>
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="number" name="first_place_points" id="first_place_points" value="{{ old('first_place_points', 5) }}" required min="0"
                    ...
                    <div>
                        ...
                        <input type="number" name="second_place_points" id="second_place_points" value="{{ old('second_place_points', 3) }}" required min="0"
                    ...
                    <div>
                        ...
                        <input type="number" name="third_place_points" id="third_place_points" value="{{ old('third_place_points', 1) }}" required min="0"
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="is_simultaneous" id="is_simultaneous" value="1" {{ old('is_simultaneous') ? 'checked' : '' }}
                           class="h-4 w-4 text-yellow-600 focus:ring-yellow-500 border-gray-300 rounded">
                    <label for="is_simultaneous" class="ml-2 block text-sm text-gray-900">
                        Competencia Simultánea (los lugares se definen en un solo enfrentamiento)
                    </label>
                </div>
                <p class="text-xs text-gray-500">Ej: Natación, donde todos compiten al mismo tiempo y se definen las posiciones directamente</p>
