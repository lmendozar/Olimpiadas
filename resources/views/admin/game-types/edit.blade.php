...
@section('title', 'Editar Tipo de Juego')
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white shadow-lg rounded-lg overflow-hidden">
            <div class="px-6 py-4 bg-blue-600">
                <h1 class="text-2xl font-bold text-white">Editar Tipo de Juego</h1>
            </div>

            <form action="{{ route('admin.game-types.update', $gameType) }}" method="POST" class="px-6 py-6 space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700">Nombre *</label>
                    <input type="text" name="name" id="name" value="{{ old('name', $gameType->name) }}" required
                ...
                <div>
                    ...
                    <select name="result_metric" id="result_metric" required
                        ...
                        <option value="tiempo" {{ old('result_metric', $gameType->result_metric) == 'tiempo' ? 'selected' : '' }}>Tiempo (s)</option>
                        <option value="goles" {{ old('result_metric', $gameType->result_metric) == 'goles' ? 'selected' : '' }}>Goles</option>
                        <option value="sets" {{ old('result_metric', $gameType->result_metric) == 'sets' ? 'selected' : '' }}>Sets</option>
                        <option value="contador" {{ old('result_metric', $gameType->result_metric) == 'contador' ? 'selected' : '' }}>Contador</option>
                    </select>
                    @error('result_metric')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-start">
                    <div class="flex items-center h-5">
                        <input type="checkbox" name="requires_individual_participants" id="requires_individual_participants" 
                               value="1" {{ old('requires_individual_participants', $gameType->requires_individual_participants) ? 'checked' : '' }}
                    ...
                    <div class="ml-3">
                        ...
                        <p class="text-sm text-gray-500">Si se marca, al crear enfrentamientos se deberá especificar qué personas específicas participan</p>
                    </div>
                </div>
