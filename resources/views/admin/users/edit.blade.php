    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white shadow-lg rounded-lg overflow-hidden">
            <div class="px-6 py-4 bg-indigo-600">
                ...
                @if($user->id === auth()->id())
                    <p class="text-indigo-100 text-sm">Estás editando tu propia cuenta</p>
                @endif
            </div>
            ...
            <form action="{{ route('admin.users.update', $user) }}" method="POST" class="px-6 py-6 space-y-6">
                ...
                <div>
                    ...
                    <select name="role" id="role" required
                            ...
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('role') border-red-500 @enderror @if($user->id === auth()->id()) bg-gray-100 @endif">
                        <option value="publico" {{ old('role', $user->role) == 'publico' ? 'selected' : '' }}>Público (Solo Lectura)</option>
                        <option value="organizador" {{ old('role', $user->role) == 'organizador' ? 'selected' : '' }}>Organizador (Administrador)</option>
                    </select>
                    @if($user->id === auth()->id())
                        <p class="mt-1 text-sm text-yellow-600">No puedes cambiar tu propio rol</p>
