...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white shadow-lg rounded-lg overflow-hidden">
            ...
            <form action="{{ route('admin.people.update', $person) }}" method="POST" class="px-6 py-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="role" id="role" required
                        ...
                        <option value="competidor" {{ old('role', $person->role) == 'competidor' ? 'selected' : '' }}>Competidor</option>
                        <option value="organizador" {{ old('role', $person->role) == 'organizador' ? 'selected' : '' }}>Organizador</option>
                    </select>
                    @error('role')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="alliance_id" id="alliance_id"
                        ...
                        @foreach($alliances as $alliance)
                            <option value="{{ $alliance->id }}" {{ old('alliance_id', $person->alliance_id) == $alliance->id ? 'selected' : '' }}>
                                {{ $alliance->name }}
                            </option>
                        @endforeach
                    </select>
