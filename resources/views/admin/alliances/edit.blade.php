    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white shadow-lg rounded-lg overflow-hidden">
            <div class="px-6 py-4 bg-green-600">
                <h1 class="text-2xl font-bold text-white">Editar Alianza</h1>
            </div>

            <form action="{{ route('admin.alliances.update', $alliance) }}" method="POST" class="px-6 py-6 space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700">Nombre *</label>
                    <input type="text" name="name" id="name" value="{{ old('name', $alliance->name) }}" required
                ...
                <div>
                    ...
                    <input type="url" name="logo_url" id="logo_url" value="{{ old('logo_url', $alliance->logo_url) }}"
                ...
                @if($alliance->logo_url)
                    <div>
                        ...
                        <img src="{{ $alliance->logo_url }}" alt="{{ $alliance->name }}" class="mt-2 h-20 w-20 rounded-full">
                    </div>
                @endif
