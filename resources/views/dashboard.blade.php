            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">🥇 Oro</th>
                        <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">🥈 Plata</th>
                        <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">🥉 Bronce</th>
                        <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Total Puntos</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <span class="text-2xl font-bold @if($index == 0) text-yellow-500 @elseif($index == 1) text-gray-400 @elseif($index == 2) text-orange-600 @else text-gray-600 @endif">
                                        #{{ $index + 1 }}
                                    </span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    @if($ranking['alliance']->logo_url)
                                        <img class="h-10 w-10 rounded-full mr-3" src="{{ $ranking['alliance']->logo_url }}" alt="{{ $ranking['alliance']->name }}">
                                    @endif
                                    <div class="text-sm font-medium text-gray-900">{{ $ranking['alliance']->name }}</div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="text-lg font-semibold text-orange-600">{{ $ranking['bronze_medals'] }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                        ...
                        <div class="flex justify-between items-start">
                            <div class="flex-1">
                                ...
                                @if($match->result_metric)
                                    <p class="text-sm text-gray-600 mt-1">Resultado: <span class="font-semibold">{{ $match->result_metric }}</span></p>
                                @endif
                                @if($match->winner)
                                    <p class="text-sm text-green-600 mt-1">🏆 Ganador: <span class="font-semibold">{{ $match->winner->name }}</span></p>
                                @endif
                            </div>
